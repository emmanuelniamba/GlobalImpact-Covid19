...
dash.register_page(__name__, path='/unemployment', name="Unemployment Analysis", order=5)
...
# Create subplot for unemployment rate over time for different countries
def create_unemployment_rate_plot(df):
    # Initialize subplots with 3 rows and 3 columns
    fig = make_subplots(rows=3, cols=3, subplot_titles=df['country'].unique())

    # Loop through the countries and plot the unemployment rate over time
    for i, country in enumerate(df['country'].unique()):
        # Filter the data for the current country
        filtered_df = df[df['country'] == country]
        ...
        # Use the index (which is the date) for the x-axis
        fig.add_trace(
            go.Scatter(x=filtered_df.index, y=filtered_df['unemployment rate'], mode='lines', name=country),
            row=(i // 3) + 1, col=(i % 3) + 1
        )

    # Update layout of the plot
    fig.update_layout(height=900, width=1200, title_text="Unemployment Rate Over Time for Different Countries")
...
layout = html.Div(children=[
    ...
    html.Br(),
    dcc.Graph(id="unemployment", figure=create_unemployment_rate_plot(df))
])
